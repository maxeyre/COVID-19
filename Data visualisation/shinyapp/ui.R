...
county_LA.list <- unique(data.county$area)
list.county <- list()
  ...
  tabPanel("Country comparison",
           ...
           h6("2. Cases and deaths shown in graphs are the number reported on that date. Delays in reporting are common internationally."),
             sidebarLayout(
               ...
               mainPanel(h5(strong("Explanation:")), 
                         ...
                         h6(strong("Cases: "),"day 0 is the first day >100 cases were reported (or 1 per 100,000 if viewing by pop. rate)"),
                         h6(strong("Deaths: "),"day 0 is the first day >10 deaths were reported (or 0.5 per 100,000 if viewing by pop. rate)"),
                         h6(strong("Global Health Security Index: "),"The ranking of each country is now included on the graph. This a measure of a country's health system capacity and 'epidemic and pandemic preparedness'"),
  ...
  tabPanel("UK overview",
           ...
           h6("3. Cases and deaths shown in graphs are the number reported on that date. Delays in reporting are common for the UK."),
                        ...
                        mainPanel(tabsetPanel(type = "tabs", id="tabs_UK",
                           ...
                           # tabPanel("England - cases", value = 2,
                           #          h3("England - cases"),
                           #          plotOutput("EnglandCasePlot")
                           #          )
                          ),
                          h6("Made by Max Eyre"),
                          h6("Any comments, questions or suggestions please contact via twitter or max.eyre@lstmed.ac.uk"),
            ...
            sidebarLayout(
             sidebarPanel(
               ...
               dateRangeInput("dateRange_county", "Date range",
                              start  = as.Date("2020-03-01","%Y-%m-%d"),
                              end    = as.Date(max(data.county$date),"%Y-%m-%d"),
                              min    = as.Date("2020-03-01","%Y-%m-%d"),
                              max    = as.Date(max(data.county$date),"%Y-%m-%d")),
